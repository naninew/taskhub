...
# [Ngày 2] Cập nhật: dùng fixture client (conftest) với DB test thật thay in-memory

import pytest


@pytest.mark.asyncio
async def test_create_label_success(client):
...
@pytest.mark.asyncio
async def test_create_label_duplicate_name_fails(client):
    # Tạo label đầu tiên
...
@pytest.mark.asyncio
async def test_get_label_detail(client):
...
@pytest.mark.asyncio
async def test_get_label_not_found(client):
